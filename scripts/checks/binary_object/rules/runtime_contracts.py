...
def check(ctx):
    ...
    apply_host = ctx.stage3b_function("src/runtime/vm_host.rs", "apply", "stage3b-apply-order")
    ...
    runtime_tests_code = ctx.rust_code_only(ctx.read_source("src/runtime/tests.rs"))
    ...
    for ctx.name, ctx.anchors in stage3b_runtime_test_contracts:
        ctx.item = ctx.stage3b_function("src/runtime/tests.rs", ctx.name, "stage3b-runtime-evidence")
        ctx.normalized_item = " ".join(ctx.item.split())
    ...
    stage3d_runtime_tests_code = ctx.stage3b_code("src/runtime/tests.rs")

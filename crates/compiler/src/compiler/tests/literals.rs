...
fn detached_string_literals_follow_quickjs_atom_identity_boundaries() {
    ...
    let quickjs_oxide_core::PrimitiveValue::String(first) = &atomized.constants[0] else {
        panic!("first atomized literal was not a String");
    };
    let quickjs_oxide_core::PrimitiveValue::String(second) = &atomized.constants[1] else {
    ...
    let quickjs_oxide_core::PrimitiveValue::String(first) = &immediate.constants[0] else {
        panic!("first immediate-atom literal was not a String");
    };
    let quickjs_oxide_core::PrimitiveValue::String(second) = &immediate.constants[1] else {
        panic!("second immediate-atom literal was not a String");
    };
    assert!(!first.same_representation(second));

    let table_backed = compile_script("['2147483648','2147483648']").unwrap();
    let quickjs_oxide_core::PrimitiveValue::String(first) = &table_backed.constants[0] else {
        panic!("first table-backed numeric literal was not a String");
    };
    let quickjs_oxide_core::PrimitiveValue::String(second) = &table_backed.constants[1] else {

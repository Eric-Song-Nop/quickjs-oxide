    ...
    for debug_info in [
        ...
        for (index, descriptor) in root.closure_variables().iter().enumerate() {
            ...
            assert!(matches!(
                ...
                Some(quickjs_oxide_core::PrimitiveValue::String(found)) if found == &bindings[index].name
            ));
        }
...
fn compiler_preserves_authenticated_with_environment_relays() {
    ...
    assert!(matches!(
        root.constants()[name as usize].as_primitive(),
        Some(quickjs_oxide_core::PrimitiveValue::String(name)) if name == &with_object.name
    ));
    ...
    let dynamically_resolves_private_name = function
        ...
        .any(|name| {
            matches!(
                ...
                Some(quickjs_oxide_core::PrimitiveValue::String(value)) if value == &JsString::from_static("named")
            )
        });
    assert!(
...
fn compiler_links_and_deduplicates_direct_eval_scope_descriptors() {
    ...
    assert!(matches!(
        ...
        Some(quickjs_oxide_core::PrimitiveValue::String(name)) if name.to_utf8_lossy() == "outerArg"
    ));

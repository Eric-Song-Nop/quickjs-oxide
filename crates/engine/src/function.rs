...
pub use quickjs_oxide_core::function::metadata;
pub use quickjs_oxide_core::function::{
    UnlinkedConstant, UnlinkedConstantError, UnlinkedFunction, UnlinkedFunctionDebug,
    UnlinkedFunctionParts, UnlinkedVariableDefinition,
};
...
mod tests {
    ...
    fn compiler_atom_string_keeps_a_structural_publication_marker() {
        ...
        assert!(matches!(
            ...
            Some(quickjs_oxide_core::PrimitiveValue::String(_))
        ));
        assert!(constant.as_child().is_none());
        let (primitive, atom_string, child) = constant.into_parts();
        assert!(matches!(
